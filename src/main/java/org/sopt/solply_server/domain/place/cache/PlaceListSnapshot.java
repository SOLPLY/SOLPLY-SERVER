 ...
 * 순서로 자리를 정하므로, 늦게 도착한 낡은 사진이 최신을 밀어내지 못한다. <b>로컬 재빌드끼리는
 * 이 가드가 할 일이 없다</b> — 재빌드가 공통 락({@link PlaceListWriteLock}) 안에서 돌아 둘이 겹치지
 * 않으므로, 발급도 {@code adopt}도 한 줄로 서고 번호가 도착 순서와 어긋날 수가 없다. 가드가
 * 지키는 것은 <b>락 밖에서 사진이 들어오는 경우</b>다 — 다중 인스턴스판에서 아카이브에서 내려받은
 ...
 * <b>읽고-고쳐-쓰기</b>라 {@code synchronized}가 붙어 있다 — 겹치면 나중 대입이 상대의 사진을
