#!/usr/bin/env bash
# 부하 라운드 동안 약 5초 간격으로 컨테이너 CPU/메모리 + MySQL 커넥션 수를 CSV 샘플링.
...
echo "ts,app_cpu,app_mem,mysql_cpu,mysql_mem,threads_connected,threads_running" > "$OUT"
END=$((SECONDS + DURATION))
while [ "$SECONDS" -lt "$END" ]; do
  STATS=$(docker stats --no-stream --format '{{.Name}},{{.CPUPerc}},{{.MemUsage}}' solply-bench-app solply-bench-mysql)
  APP=$(echo "$STATS" | grep 'bench-app' | cut -d, -f2,3)
