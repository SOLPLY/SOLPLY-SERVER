...
> **되돌림(2026-09-10). 아래 streaming 구현은 지금 코드에 없다.** 10배(63,200 장소)·보존 3장이 찬
> 상태의 같은 창 측정(`docs/perf/2026-09-10-rebuild-headroom-x10.md`)에서 streaming과
> `getResultList()`가 갈린 것은 발화 스레드 할당 240.0 → 254.2 MB(+5.9%) 한 항목뿐이고, 소요·CPU·힙·GC는
> 드리프트 안이라 구분되지 않았다. 대가는 셋이다 — 드라이버 전용 fetch size 약속(`Integer.MIN_VALUE`),
> "스트림을 닫기 전 같은 커넥션에 다른 문장 금지"라는 정확성 계약(문장 ①·③의 순서가 조건이 된다),
> 서버 prepared statement 모드에서 그 계약을 어기면 예외가 아니라 소켓 읽기에서 멈추는 함정. 현실적
> 최대 규모에서 이득이 측정되지 않아 단순한 쪽으로 되돌렸다. 100배(옛 로더 형상)에서 Full GC를 정상
> 상태 2 → 1로 줄인 실측은 "재빌드 CPU·할당이 부담이 될 때의 첫 손잡이"로 남는다. 결정 문서는
> `docs/design/2026-09-10-inmemory-snapshot-decision.md` §8, 구현은 커밋 `78abdac`.
> 아래 본문(바이트코드 사슬, 기각 근거, 측정)은 그대로 기록이다.
...
- 100배 — 성공 조건 충족(Full GC 4 → 1, peak heap 상한 → 1,198 MB). 그러나 변경 후도 **셋째
  ...
  정점을 정한다)은 지지된다.
  - **보정(2026-09-10).** 발화 1·2는 보존 1·2장의 워밍업 상태라 운영 조건 표본은 셋째 발화
    1건뿐이다. 정상 상태끼리는 Full GC 2 → 1이고, peak heap은 둘 다 상한이라 구분 불가이며,
    차이는 Full GC 직후 생존 크기(815~929 → 720 MB)에 남는다. "4 → 1"은 워밍업이 섞인 합이다.
    근거는 정본 §4.1 보정 문단, 3장 찬 상태의 표본 5회 재측정은 2026-09-10 캠페인(10배).
...
- **streaming을 다시 켤 신호: 재빌드 창의 스로틀이 발화마다 붙거나, 재빌드 할당이 Eden 여러 번
  분량이 될 때.** 되돌린 이유는 머리의 표식, 꺼내는 순서는 결정 문서 §7 A의 2단계.
