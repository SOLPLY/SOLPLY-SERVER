...
public class PlaceListJoinOrderPolicy {
     ...
     *
     * <p><b>⚠️ 판단을 지배하는 변수는 비율이다: 유효 태그 규모 ÷ 지역 후보 수 (전환점 ≈ 0.4~0.64).</b>
     * 이 규칙은 그중 분자만 읽고, 분모는 "복수 동네 조회 ≈ 후보 1,800"이라는 현재 데이터의 전제로
     * 이 상수에 고정해 두었다. 분모를 읽지 않는 이유는 지역 크기 축을 1,800 한 점에서만 쟀기
     * 때문이다. 따라서 장소 수가 자릿수로 변하거나 복수 동네 조회의 후보 수가 1,800과 크게
     * 달라지는 구성이 생기면, 이 상수는 조정 대상이 아니라 재측정 대상이다.
     */
