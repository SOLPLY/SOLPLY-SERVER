...
public class PlaceService {
   ...
   * 매번, 태그 검증({@code TagValidator})이 {@code mainTagId}가 있을 때, {@code isBookmarked}가
    ...
    List<PlacePreviewDto> previews = displayed.stream()
        .map(row -> {
          ...
          return PlacePreviewDto.of(
              ...
              entry.bookmarkCount(),
              entry.reviewCount(),
              // 엔트리는 무척도 정수만 든다 — 스케일 2를 여기서 되씌워 컬럼 값과 같은 BigDecimal을 낸다
              BigDecimal.valueOf(entry.ratingToInt(), 2));
        })
        .toList();
   ...
   * 필터는 장소가 가진 <b>모든 태그의 id·타입·활성</b>을 본다. 엔트리가 담는 것은 대표 태그
   * <em>이름</em>과 마스크뿐이라 여기서는 쓸 수 없고, 쓰려면 태그 집합을 통째로 실어야 하는데 그러면
   * "목록 한 항목이 쓰는 값만 담는다"는 엔트리의 정의가 무너진다. 페이징이 없어 id 목록의 상한이
