 ...
 *
 * <p><b>불변식: 행이 있는 장소 = 마지막 카운트 배치 시점의 활성 장소.</b> 인기순 조회가 places를
 * 되짚지 않는 근거다. 지키는 주체는 카운트 배치의 {@code WHERE p.active = 1} + 잔행 삭제이고,
 * 대가는 노출 창 ≤1h다 — 비활성화도 재활성화도 다음 카운트 배치까지 반영되지 않는다.
 *
