...
/**
 * {@link PlaceListSnapshotLoader#rebuild()}를 부르는 <b>유일한</b> 트리거 — 기동 한 번과 10분 주기다.
 *
 * <p><b>이 캐시는 "DB의 10분 지연 읽기 모델"이다.</b> 쓰기 경로는 캐시를 모른다 — 어드민도 배치도
 * RDB만 만지고, 그 변경이 목록에 나타나는 것은 다음 회차가 사진을 다시 찍을 때다. 그래서 SLA는
 * <b>"어드민 변경은 최대 10분 안에 반영"</b>이고, 비활성화한 장소가 최대 10분간 목록에 남는 창은
 * <b>수용 항목</b>이다. 반영을 앞당기려면 주기를 줄이는 것이 유일한 손잡이이며, 쓰기 경로에 무효화
 * 훅을 다는 방향은 이 설계가 일부러 버린 쪽이다(훅 하나가 빠지면 그 경로만 조용히 낡는다).
 *
