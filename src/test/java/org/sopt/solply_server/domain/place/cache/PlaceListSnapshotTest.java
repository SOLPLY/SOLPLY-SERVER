 ...
 *
 * <p><b>버전을 손으로 찍는 이유.</b> 운영에서 번호를 받아 사진에 붙이는 것은 빌더 하나이고
 * ({@code PlaceListSnapshotLoader#rebuild}) 홀더는 완성된 사진을 받기만 한다. 그래서 여기서는
 * 발급소를 세울 것 없이 회차 번호를 직접 정해 넣는다 — 늦게 도착한 낡은 사진처럼 실제 발급으로는
...
class PlaceListSnapshotTest {
    ...
    /**
     * <b>늦게 도착한 낡은 사진은 최신을 밀어내지 못한다.</b> 번호가 빌드를 <em>끝낸</em> 순서로
     * 발급되므로, 어드민 훅 빌드와 타이머 빌드가 겹치면 먼저 번호를 받은 사진이 나중에 홀더에
