 ...
 * 어드민이 스크롤 유지를 고른 회차는 revision만 오르고 cursorVersion은 그대로다. 새것인지의 판정은
 * 언제나 {@link SnapshotMetadata#isNewerThan}이다 — 회차가 오르면 revision이 0으로 리셋되기
 * 때문이다.
 *
 ...
 *
 * @param metadata     이 스냅샷이 읽은 원본 시점의 번호 둘
 * @param sortedPlaces 그 시점의 완결된 불변 정렬 배열
 */
public record Snapshot(SnapshotMetadata metadata, SortedPlaces sortedPlaces) {
}
