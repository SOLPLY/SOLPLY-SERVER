...
모든 작업은 아래 순서를 따르며, Claude는 스킬을 사용하여 이를 자율적으로 제안하고 수행한다.

- GitHub Issue 생성 (create-issue 스킬)
- 이슈 기반 브랜치 생성 (create-branch 스킬)
- 코드 구현 
- 커밋 작성 (create-commit 스킬)
- Pull Request 오픈 (create-pr 스킬)

### Rules
- GitHub Issues: .github/ISSUE_TEMPLATE 내 bug.yml, feature.yml, refactor.yml을 준수하며 한국어로 작성한다. 
- Branch Naming: <type>/#<issue-number>-<short-description> (예: feat/#339-recommend-place)
- Commit Message: #<issue-number> <type>: <description> (예: #339 feat: 임베딩 로직 구현)
...
🛠 Skills
Claude는 .claude/skills에 정의된 스킬을 사용하여 이슈 생성, 브랜치 관리, 커밋 및 PR 생성을 수행한다.