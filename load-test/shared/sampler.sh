...
# 앱 1대 + edge 구성 대응 (2026-08-05. 그전에는 앱 2대 + LB였고 CSV에 app1/app2 두 칸이었다 —
# 옛 캠페인의 samples.csv와 컬럼이 다르므로 두 세대를 같은 스크립트로 파싱하지 말 것).
set -uo pipefail
...
CONTAINERS=(solply-bench-app solply-bench-lb solply-bench-mysql)
mkdir -p "$(dirname "$OUT")"
...
echo "ts,app_cpu,app_mem,edge_cpu,mysql_cpu,mysql_mem,threads_connected,threads_running" > "$OUT"
END=$((SECONDS + DURATION))
while [ "$SECONDS" -lt "$END" ]; do
  STATS=$(docker stats --no-stream --format '{{.Name}},{{.CPUPerc}},{{.MemUsage}}' "${CONTAINERS[@]}" 2>/dev/null || echo "")
  APP=$(field "$STATS" 'bench-app'   2,3)
  EDGE=$(field "$STATS" 'bench-lb'   2)
  ...
  echo "$(date +%T),$APP,$EDGE,${DB},${TC:-NA},${TR:-NA}" >> "$OUT"
  sleep 3
done
echo "[sampler] 저장: $OUT"
