 ...
 * 그래서 커서가 <b>자기가 시작한 회차의 버전</b>을 싣고 다니고, 서버는 그 버전의 스냅샷으로만 이어
 * 서빙한다. 보존은 캐시가 들고 있는 최근 3장뿐이며({@code SnapshotBox}) 그 밖의 버전은
 * {@code EXPIRED_PLACE_CURSOR}로 명시 만료된다. 번호 자체는 DB 발급 테이블에서 나오지만
 * ({@code SnapshotVersionIssuer}) 그 번호가 가리키는 <b>스냅샷</b>은 그것을 지은 인스턴스의
 * 메모리에만 있다.
 *
