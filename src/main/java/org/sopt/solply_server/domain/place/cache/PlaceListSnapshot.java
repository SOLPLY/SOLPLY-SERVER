 ...
 * 이유는 <b>버전이 빌드가 끝난 시각에 찍히기 때문</b>이다 — 어드민 훅 빌드와 타이머 빌드가 겹치면
 * 먼저 시작해 늦게 끝난 쪽이 더 큰 버전을 갖는 것이 정상이지만, 먼저 <em>끝나</em> 버전을 받은
 * 사진이 스레드 스케줄에 밀려 나중에 {@code adopt}에 도착하는 역전은 남는다. 그 한 창을 이 가드가
 * 닫는다. 확장 쪽 이유도 같은 모양이다 — 다중 인스턴스판에서 아카이브에서 내려받은 <b>남의 사진</b>이
 * 들어오는 곳도 이 진입점 하나이고, 그때 "받은 버전이 로컬 최신보다 새면 채택"이라는 채택 규칙이
 * 곧 이 가드다({@code docs/design/2026-09-01-multi-instance-snapshot-pipeline.md} §3-4).
 *
...
public class PlaceListSnapshot {
     ...
     * </ul>
     *
     * <p>버전은 빌드가 끝난 시각(ms)이며 여기서 찍지 않는다({@link PlaceListSnapshotLoader#rebuild()}).
     * 어드민 훅이 붙어 간격이 분 단위가 아니게 된 지금도 <b>같은 값이 두 번 나오지 않는</b> 근거는
     * 빌드 자체의 길이다 — 전량 재생성이 수십~수백 ms라 두 회차가 같은 밀리초에 끝날 수 없다
     * (실측 6,320개 94~302ms). 재기동해도 이전 인스턴스가 찍던 값보다 크다 — 다만 재기동하면 보존
     * 목록 자체가 비므로 옛 커서는 만료된다.
     *
