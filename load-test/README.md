...
| 캠페인 | 증명하려는 주장 | 결과 | 상태 |
|---|---|---|---|
| [`2026-08-02_post-removal-regression`](campaigns/2026-08-02_post-removal-regression/) | ① 캐시 제거 후 실코드가 목표 부하(120 req/s)를 회귀 없이 처리하는가 ② 피크 중 배치 발화가 무해한가 ③ 읽기·쓰기 혼합에서 한계 규모와 먼저 포화하는 자원은 무엇인가 | ① 3/4 (p50 7.9>7.2 — 워밍 부족 정황, 판정 유지) ② 충족 4/4 (배치 1.08배·p50 0.76배) ③ 240 통과·300에서 MySQL CPU 포화. **단 쓰기 믹스 미재현(프로세서 placeId 결함) — 읽기 전용 한계로만 인용** → [결과 문서](../docs/perf/2026-08-02-post-removal-regression.md) | ⚠️ 부분 충족 (형상 `e6ac941`) |
