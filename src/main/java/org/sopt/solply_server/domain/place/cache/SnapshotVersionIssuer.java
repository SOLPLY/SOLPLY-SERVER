...
/**
 * 회차 스냅샷({@link Snapshot})에 붙일 <b>버전 번호를 내주는 곳</b>. 하는 일은
 ...
 * 시계가 느린 쪽이 더 새 데이터로 지은 스냅샷이 작은 번호를 달고 {@link SnapshotBox#adopt}의
 ...
 * ({@link SnapshotLoader#rebuild()}·{@link SnapshotLoader#patch})은 {@code readOnly = true}
 * 트랜잭션에서 이어지는 자리이고, MySQL은 읽기 전용 트랜잭션 안의 INSERT를 거부한다. 그래서 발급은
 * 자기 트랜잭션을 열어 INSERT 하나로 즉시 커밋한다. 같은 클래스의 메서드로 두면 자기 호출이라
 * 프록시를 타지 않아 이 전파가 조용히 무시되므로, 빈을 나누는 것이 계약이다. 값을 치르는 것은
 * 커넥션이다 — 읽기가 아직 열려 있는 채로 발급하면 커넥션 <b>둘</b>을 동시에 쥐므로, 로더는 읽기를
 * 닫은 뒤에 발급한다.
 *
 * <p><b>발급을 부르는 것 자체가 판정이다.</b> 어드민 부분 패치는 정렬 배열이 실제로 달라졌을 때만
 * 여기까지 온다 — 이름·이미지처럼 표시값만 바뀐 수정은 발급을 건너뛴다. 번호에 빈틈이 생기는 것은
 * 상관없지만(단조면 족하다) <b>내용이 같은데 번호만 새로 찍히는 것</b>은 곧 진행 중인 스크롤의
 * 만료다.
 *
 * <p><b>폴백을 두지 않는다.</b> DB가 번호를 못 주면 예외를 그대로 올려 재빌드를 실패시킨다
 * (직전 스냅샷이 그대로 남는 기존 정책). 실패했을 때 밀리초 시각으로 대신 찍으면 번호 공간이 둘로
 * 섞여, 한 번의 폴백이 그 뒤 실제 발급 번호를 전부 "낡은 버전"으로 만든다.
 */
@Component
@RequiredArgsConstructor
public class SnapshotVersionIssuer {
    ...
    /**
     * 번호 하나를 발급한다. 한 스냅샷에 한 번만 부르는 것이 "버전↔내용 1:1" 불변식의 전제다.
     *
