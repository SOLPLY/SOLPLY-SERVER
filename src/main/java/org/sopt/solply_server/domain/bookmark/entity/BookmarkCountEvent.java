 ...
 * 하나뿐이며, 배치는 적용과 동시에 자기가 표시한 행을 지운다.
 *
 * <p><b>{@code consumption_id}(V42)를 여기에 매핑하지 않는다.</b> 그 칸의 주인은 소비
 * 트랜잭션이고 값도 네이티브 문장으로만 오간다 — 엔티티에 실으면 쓰기 경로의 INSERT가
 * 쓸 일 없는 칸을 하나 더 싣는다.
 *
 * <p><b>{@code createdAt}은 소비 로직이 쓰지 않는다.</b> 삭제 대상은 오로지 표식이 정한다.
 * 이 값은 "전표가 얼마나 밀렸는가"를 운영에서 눈으로 보기 위한 칸이다. 시각으로
