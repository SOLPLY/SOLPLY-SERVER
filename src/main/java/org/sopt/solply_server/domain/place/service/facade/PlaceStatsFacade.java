 ...
 * 넘기면 :30 델타는 전표 잠금 뒤에서 기다리고, 아래 단일 스레드 때문에 그 뒤 회차들도 그만큼
 * 밀린다. {@code lockAtMostFor PT10M}은 이 5분보다 길다. 간격이 실제로 지켜지는지는 회차 종료
 * 로그의 {@code elapsed}로 본다 — <b>주기나 재시도 설정을 바꿀 때 이 간격을 함께 확인할 것.</b>
 ...
 *
 * <p><b>{@code @Scheduled} 기본 실행기는 단일 스레드다.</b> 네 회차와 스냅샷 폴이 한 스레드를
 * 나눠 쓰므로 앞 회차가 길어지면 뒤 회차의 발화가 그만큼 밀린다. <b>밀려서 건너뛴 발화는 아무 데도
 * 남지 않는다</b> — 되짚는 장치가 없으므로 다음 발화까지 값이 낡는다.
 *
...
public class PlaceStatsFacade {
     ...
     *
     * <p>예외를 스케줄러 스레드로 흘리지 않는다 — 기본 실행기가 단일 스레드라 이후 회차의 발화까지
     * 함께 잃는다.
     *
