 ...
 * 그 일을 회차가 대신한다.
 *
 * <p><b>04:40인 이유는 다른 회차와 시각을 가르기 위해서다.</b> 01:25 카운트 안전망, 03:00 장소
 * 임베딩, 04:00 코스 임베딩, 그리고 15분 격자의 집계 회차들(매시 :00 :05 :10 :20 …)이 이미 있다.
 * 회차들은 배치 풀에서 병렬로 돌 수 있으므로({@code SchedulingConfig}) 겹침이 스레드 대기를
 * 만들지는 않는다. 시각을 가르는 것은 <b>같은 테이블을 두고 DB에서 잠금을 다투지 않게</b> 하려는
 * 것이다.
 *
 ...
 * 실패해도 앞서 지운 것은 이미 커밋돼 있고 다음 회차가 남은 것을 이어서 지운다.
 *
 * <p><b>예외를 여기서 잡는 이유는 로그 등급이지 스케줄러 스레드가 아니다.</b> 스프링은 반복 작업의
 * 예외를 로그로 삼키고 다음 발화를 이어 가므로, 흘려보내도 스케줄이 죽지는 않는다 — 달라지는 것은
 * 이 회차가 남기는 줄의 등급과 맥락뿐이다.
 */
@Slf4j
@Component
