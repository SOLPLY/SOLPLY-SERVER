 ...
 * 회차를 계속 보게 하는 장치다. 회차 간격이 10분이므로 최신 + 직전 2장은 <b>20~30분</b>의 스크롤을
 ...
 *
 * <p>{@code volatile}이 하는 일은 하나다 — 스케줄러 스레드가 대입한 새 목록을 요청 스레드가
 * 반드시 보게 한다. 목록도 인덱스도 불변이라 그 뒤의 동기화는 필요 없다. 읽고-고쳐-쓰는
 * {@link #replace}에 락이 없는 근거는 <b>쓰는 스레드가 스케줄러 하나뿐</b>이라는 것이다.
 */
@Component
public class PlaceListSnapshot {
     ...
     * 부분 갱신 진입점은 의도적으로 없다.
     *
     * <p>버전은 교체 시각(ms)이다. 회차 간격이 분 단위라 같은 값이 두 번 나올 수 없고, 재기동해도
     * 이전 인스턴스가 찍던 값보다 크다 — 다만 재기동하면 보존 목록 자체가 비므로 옛 커서는 만료된다.
     */
