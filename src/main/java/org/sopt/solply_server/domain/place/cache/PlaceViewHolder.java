 ...
 * </ul>
 * 둘의 순서를 가리는 것은 설치자가 들고 있는 <b>관측 revision</b>이고, 그 규칙은
 * {@link SnapshotInstaller} javadoc에 있다.
 *
 * <p><b>지운 장소는 그 자리에서 사라진다.</b> 패치가 다시 읽었을 때 원본에 행이 없으면
 * {@link #remove}로 표시값을 지우고, 조회 경로가 표시값 없는 행을 건너뛴다. 정렬 배열에서 빠지는
 * 것은 다음 리빌드의 몫이다.
 *
 * <p><b>조회는 락을 잡지 않는다.</b> {@link #get}은 {@code volatile} 참조 한 번과
 * {@code ConcurrentHashMap} 읽기 한 번이 전부다. 반대로 쓰기
 * ({@link #replaceAll}·{@link #put}·{@link #remove})는 {@link CacheWriteLock} 안에서만 불러야
 * 한다 — 그 이유는 그쪽 javadoc.
 *
...
public class PlaceViewHolder {
    ...
    /** 리빌드 설치는 이 참조를 통째로 갈고, 커밋 직후 패치는 가리키는 맵의 항목만 고친다 */
     ...
     * 않아야 한다. 복사본을 하나 더 뜨면 전 장소의 표시값을 리빌드마다 두 번 담게 되는데, 그것을
     * 아끼려고 만드는 쪽이 처음부터 홀더가 쓸 맵을 만들어 준다
     * ({@code SnapshotLoader#readSourceState}).
     *
     * <p>파라미터가 {@link ConcurrentMap}인 것도 그래서다. 패치({@link #put}·{@link #remove})가 이
     * 맵의 항목을 직접 고치므로 일반 {@code HashMap}이 들어오면 조회와 수정이 겹치는 순간
     * 깨진다 — 타입으로 막아 둔다.
     */
    ...
    /** 어드민이 손댄 장소 하나 — 그 항목만 갈아 끼운다 ({@link SnapshotViewPatcher}) */
    void put(PlaceView view) {
        views.put(view.placeId(), view);
    }

    /**
     * 표시값을 지운다 — 그 장소가 목록 원본에서 사라졌다는 뜻이다.
     *
     * <p>정렬 배열에서 빼는 것은 다음 리빌드의 몫이지만, 조회 경로가 표시값 없는 행을 건너뛰므로
     * 이것만으로 목록에서 즉시 사라진다.
     */
    void remove(long placeId) {
        views.remove(placeId);
    }

    /**
     * 전량 읽기. <b>지금 부르는 곳이 없다</b> — 표시값 전량을 자바로 꺼내 쓰던 자리는 스냅샷을
     * 직렬화해 공유하던 시절의 것이고, 리빌드가 원본에서 직접 읽는 지금은 남아 있지 않다.
     *
     * <p>남겨 둔다면 이 모양이어야 한다는 것도 함께 적어 둔다 — 살아 있는 맵을 그대로 넘기면
     * 읽는 도중 {@link #put}·{@link #remove}가 끼어들므로 불변 사본을 준다.
     */
